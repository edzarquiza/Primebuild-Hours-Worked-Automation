...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Dexterous | Hours Worked Automation",
    page_icon="⏱️",
    layout="wide",
)

# ── Brand colours (Dexterous) ─────────────────────────────────────────────────
# Background: #0a0a0a  Surface: #141414  Card: #1c1c1c
# Teal: #00c4b4  Blue: #0072ff  Text: #ffffff  Muted: #8a8a8a
...
.dex-header img { height: 44px; }
.dex-header-text h1 {
    margin: 0; font-size: 1.45rem; font-weight: 700;
    background: linear-gradient(90deg, #00c4b4, #0072ff);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}
.dex-header-text p {
    margin: .25rem 0 0; color: #8a8a8a !important;
    -webkit-text-fill-color: #8a8a8a !important; font-size: .9rem;
}
...
.metric-card .lbl {
    font-size: .8rem; color: #8a8a8a !important;
    -webkit-text-fill-color: #8a8a8a !important; margin-top: .25rem;
}
.metric-card.warn  .num { background: linear-gradient(90deg,#f59e0b,#f97316); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
.metric-card.danger .num { background: linear-gradient(90deg,#ef4444,#dc2626); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
...
[data-testid="stInfo"]    { background:#162230 !important; border-color:#0072ff !important; color:#ffffff !important; }
[data-testid="stSuccess"] { background:#0d2318 !important; border-color:#00c4b4 !important; color:#ffffff !important; }
[data-testid="stError"]   { background:#2a1010 !important; border-color:#ef4444 !important; color:#ffffff !important; }
[data-testid="stWarning"] { background:#2a1f00 !important; border-color:#f59e0b !important; color:#ffffff !important; }
...
.stDownloadButton > button, .stButton > button {
    background: linear-gradient(90deg, #00c4b4, #0072ff) !important;
...
[data-testid="stFileUploader"] * { color: #ffffff !important; -webkit-text-fill-color:#ffffff !important; }
...
[data-testid="stExpander"] summary { color: #ffffff !important; -webkit-text-fill-color:#ffffff !important; }

/* ── Divider ── */
hr { border-color: #1e1e1e !important; }

/* ── Spinner text ── */
[data-testid="stSpinner"] p { color:#8a8a8a !important; -webkit-text-fill-color:#8a8a8a !important; }

/* ── Dataframe / table — fix invisible text on dark bg ── */
[data-testid="stDataFrame"] * { color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; }
[data-testid="stDataFrame"] canvas { color: #ffffff !important; }
.dvn-scroller, .dvn-scroller * { color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; }
...
/* Streamlit's internal dataframe wrapper */
[data-testid="stDataFrameResizable"] { background: #1c1c1c !important; }
iframe[data-testid] { background: #1c1c1c !important; }
/* Force all text inside dataframe containers */
...
# Logo lives in assets/logo.jpg (committed to repo — works on Streamlit Cloud)
import os as _os
_logo_path = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "logo.jpg")
try:
    _logo_b64 = _img_b64(_logo_path)
    _logo_html = f'<img src="data:image/jpeg;base64,{_logo_b64}" />'
except Exception:
    _logo_html = '<span style="font-size:1.4rem;font-weight:700;color:#00c4b4">dexterous</span>'

st.markdown(f"""
<div class="dex-header">
  {_logo_html}
  <div class="dex-header-text">
    <h1>Hours Worked Automation</h1>
    <p>Upload one or more weekly timesheet exports — compliance reports generated instantly.</p>
  </div>
</div>
""", unsafe_allow_html=True)
...
def _render_table(df: pd.DataFrame):
    """Render a dataframe as a styled HTML table that works on dark backgrounds."""
    styles = """
    <style>
    .dex-table { width:100%; border-collapse:collapse; font-size:.88rem; margin-top:.5rem; }
    .dex-table th {
        background:#1e1e1e; color:#00c4b4 !important; -webkit-text-fill-color:#00c4b4 !important;
        padding:.6rem .9rem; text-align:left; border-bottom:1px solid #2e2e2e;
        font-weight:600; white-space:nowrap;
    }
    .dex-table td {
        padding:.55rem .9rem; border-bottom:1px solid #1e1e1e;
        color:#ffffff !important; -webkit-text-fill-color:#ffffff !important;
        background:#141414;
    }
    .dex-table tr:hover td { background:#1c1c1c; }
    </style>
    """
    html = styles + df.to_html(classes="dex-table", index=False, escape=False)
